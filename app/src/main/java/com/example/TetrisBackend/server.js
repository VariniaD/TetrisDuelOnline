...
io.on("connection", (socket) => {
    ...
    socket.on("send_attack", ({ roomId, garbageLines }) => {
        ...
        console.log(`Attack from ${socket.id} to room ${roomId}: ${garbageLines} lines`);
        if (!garbageLines || typeof garbageLines !== "number" || garbageLines <= 0) {
            socket.emit("error_message", {
                message: "Invalid garbage lines"
            });
            return;
        }
